<x-app-layout>
    <div class="max-w-2xl mx-auto space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Tambah User Baru</h1>
                <p class="text-sm text-slate-500 mt-0.5">Buat akun baru untuk tim CS atau admin</p>
            </div>
        ...
        <form action="{{ route('users.store') }}" method="POST" class="bg-white rounded-2xl border border-slate-200 p-6 space-y-5">
            @csrf
            ...
            <div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <label class="cursor-pointer">
                        <input type="radio" name="role" value="cs" {{ old('role', 'cs') === 'cs' ? 'checked' : '' }} class="peer sr-only" required>
